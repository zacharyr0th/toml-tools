...
def main() -> None:
    ...
    input_path = os.path.join('input', f"{args.toml_file}.toml")
    if not os.path.exists(input_path):
        logging.error("File %s not found.", input_path)
        sys.exit(1)

    output_filename = f"{args.toml_file}-stats-{current_date}.txt"
    output_path = os.path.join('output', output_filename)
    generate_stats(input_path, output_path)
    logging.info("Generated stats for %s.toml -> %s", args.toml_file, output_filename)
